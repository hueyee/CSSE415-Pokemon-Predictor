...
def fetch_replays(format="gen3ou", pages=1):
    ...
    base_url = f"https://replay.pokemonshowdown.com/search.json?format={format}&page="
    all_replays = []
...
def download_replay(replay_id, player1, player2, stats):
...
def main():
    ...
    for replay in replays:
        ...
        if replay_id and len(players) == 2 and rating is not None:
            ...
            file_size = download_replay(replay_id, player1, player2, stats)
        else:
            stats["skipped"] += 1
            file_size = None
