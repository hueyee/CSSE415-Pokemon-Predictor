...
def create_prediction_process_visualization(models, sample_row):
    plt.figure(figsize=(16, 12))

    current_state = sample_row.copy()
    positions = list(range(2, 7))
    prediction_history = []
    state_history = []
    ...
    for i, pred_info in enumerate(prediction_history):
        ax = axes[i]
        pokemon_names = [p[0][:12] for p in pred_info['predictions']]
        confidences = [p[1] for p in pred_info['predictions']]

        bars = ax.bar(range(len(pokemon_names)), confidences,
                      color=['darkgreen', 'green', 'lightgreen', 'orange', 'red'])
        ax.set_title(f'Position {pred_info["position"]} Predictions', fontsize=14, fontweight='bold')
        ax.set_ylabel('Confidence')
        ax.set_xticks(range(len(pokemon_names)))
        ax.set_xticklabels(pokemon_names, rotation=45, ha='right')
        ax.set_ylim(0, max(confidences) * 1.1)
    ...
    plt.tight_layout()
    plt.savefig('prediction_process_flow.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Prediction process flow visualization saved to prediction_process_flow.png")

def create_state_evolution_visualization(models, sample_row):
    current_state = sample_row.copy()
    positions = list(range(2, 7))

    fig, ax = plt.subplots(figsize=(14, 8))

    revealed_counts = [current_state['p2_number_of_pokemon_revealed']]
    turn_ids = [current_state.get('turn_id', 0)]
    position_labels = ['Start']

    for position_idx in positions:
        predictions = predict_next_pokemon(current_state, position_idx, models[position_idx])
        top_pokemon, confidence = max(predictions.items(), key=lambda x: x[1])
        ...
        revealed_counts.append(current_state['p2_number_of_pokemon_revealed'])
        turn_ids.append(current_state['turn_id'])
        position_labels.append(f'P{position_idx}')

    ax.plot(range(len(revealed_counts)), revealed_counts, 'bo-', linewidth=2, markersize=8, label='Pokemon Revealed')
    ax2 = ax.twinx()
    ax2.plot(range(len(turn_ids)), turn_ids, 'ro-', linewidth=2, markersize=8, label='Turn ID')

    ax.set_xlabel('Prediction Step')
    ax.set_ylabel('Pokemon Revealed', color='blue')
    ax2.set_ylabel('Turn ID', color='red')
    ...
    plt.title('Game State Evolution During Prediction Process', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig('state_evolution.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("State evolution visualization saved to state_evolution.png")

def create_confidence_distribution_visualization(all_predictions):
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))

    prediction_df = pd.DataFrame(all_predictions)

    axes[0, 0].hist(prediction_df['confidence'], bins=30, alpha=0.7, color='skyblue', edgecolor='black')
    axes[0, 0].set_title('Distribution of Prediction Confidences')
    axes[0, 0].set_xlabel('Confidence')
    axes[0, 0].set_ylabel('Frequency')
    axes[0, 0].axvline(prediction_df['confidence'].mean(), color='red', linestyle='--',
                       label=f'Mean: {prediction_df["confidence"].mean():.3f}')
    axes[0, 0].legend()

    conf_vs_acc = prediction_df.groupby(pd.cut(prediction_df['confidence'], bins=10))['correct'].agg(['mean', 'count']).reset_index()
    conf_vs_acc['confidence_mid'] = conf_vs_acc['confidence'].apply(lambda x: x.mid)
    axes[0, 1].scatter(conf_vs_acc['confidence_mid'], conf_vs_acc['mean'],
                       s=conf_vs_acc['count']*2, alpha=0.6, color='green')
    axes[0, 1].plot(conf_vs_acc['confidence_mid'], conf_vs_acc['mean'], 'r--', alpha=0.8)
    axes[0, 1].set_title('Confidence vs Accuracy (bubble size = count)')
    axes[0, 1].set_xlabel('Confidence')
    axes[0, 1].set_ylabel('Accuracy')

    pokemon_acc = prediction_df.groupby('pokemon_idx')['correct'].mean()
    axes[1, 0].bar(pokemon_acc.index, pokemon_acc.values, color='lightcoral', alpha=0.7)
    axes[1, 0].set_title('Accuracy by Pokemon Position')
    axes[1, 0].set_xlabel('Pokemon Position')
    axes[1, 0].set_ylabel('Accuracy')
    for i, v in enumerate(pokemon_acc.values):
        axes[1, 0].text(pokemon_acc.index[i], v + 0.01, f'{v:.3f}', ha='center')

    order_acc = prediction_df.groupby('propagation_order')['correct'].agg(['mean', 'count'])
    axes[1, 1].bar(order_acc.index, order_acc['mean'], color='gold', alpha=0.7)
    axes[1, 1].set_title('Accuracy by Propagation Order')
    axes[1, 1].set_xlabel('Propagation Order')
    axes[1, 1].set_ylabel('Accuracy')
    for i, (idx, row) in enumerate(order_acc.iterrows()):
        axes[1, 1].text(idx, row['mean'] + 0.01, f'{row["mean"]:.3f}\n(n={row["count"]})', ha='center')

    plt.tight_layout()
    plt.savefig('confidence_distribution_analysis.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Confidence distribution analysis saved to confidence_distribution_analysis.png")

def create_pokemon_prediction_matrix(all_predictions):
    prediction_df = pd.DataFrame(all_predictions)

    top_predicted = prediction_df['predicted'].value_counts().head(20).index
    top_actual = prediction_df['actual'].value_counts().head(20).index
    common_pokemon = list(set(top_predicted).union(set(top_actual)))

    filtered_df = prediction_df[prediction_df['predicted'].isin(common_pokemon) &
                                prediction_df['actual'].isin(common_pokemon)]

    confusion_matrix = pd.crosstab(filtered_df['actual'], filtered_df['predicted'], normalize='index')

    plt.figure(figsize=(16, 14))
    sns.heatmap(confusion_matrix, annot=False, cmap='YlOrRd', fmt='.2f')
    plt.title('Pokemon Prediction Confusion Matrix (Top 20 Most Common)')
    plt.xlabel('Predicted Pokemon')
    plt.ylabel('Actual Pokemon')
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig('pokemon_prediction_matrix.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Pokemon prediction matrix saved to pokemon_prediction_matrix.png")
...
def debug_prediction_process(models):
    ...
    print("\n===== DEBUG COMPLETE =====")
    return current_state, sample_row
...
def main():
    ...
    if SAMPLE_SIZE is not None:
        ...
        for strata_value, sample_count in samples_per_stratum.items():
            ...
            if len(strata_df) > 0:
                sampled = strata_df.sample(min(sample_count, len(strata_df)), random_state=RANDOM_STATE)
    ...
    debug_state, sample_row = debug_prediction_process(models)

    print("Creating additional visualizations...")
    create_prediction_process_visualization(models, sample_row)
    create_state_evolution_visualization(models, sample_row)
    ...
    create_confidence_distribution_visualization(all_predictions)
    create_pokemon_prediction_matrix(all_predictions)
