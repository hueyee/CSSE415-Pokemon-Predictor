...
FILE_PATH = "../Parquets/validation_pokemon_moves.csv"
MODELS_DIR = "../Models"
...
def create_prediction_evolution_visualization(df, models, sample_size=5):
    print("\nCreating prediction evolution visualization...")

    fig, axes = plt.subplots(sample_size, 1, figsize=(15, 6*sample_size))
    if sample_size == 1:
        axes = [axes]

    sample_data = df.sample(n=min(sample_size, len(df)), random_state=RANDOM_STATE)

    for idx, (_, row) in enumerate(sample_data.iterrows()):
        ax = axes[idx]

        initial_state = row.to_dict()
        initial_revealed = initial_state.get('p2_number_of_pokemon_revealed', 0)

        start_idx = max(2, initial_revealed + 1)
        if start_idx > 6:
            ax.text(0.5, 0.5, f"Game {row.get('game_id', 'Unknown')}: All Pokemon revealed",
                    ha='center', va='center', transform=ax.transAxes)
            continue

        prediction_chain, _ = propagate_predictions(initial_state, models, start_idx)

        if not prediction_chain:
            ax.text(0.5, 0.5, f"Game {row.get('game_id', 'Unknown')}: No predictions generated",
                    ha='center', va='center', transform=ax.transAxes)
            continue
        ...
        for stage_idx, pred_step in enumerate(prediction_chain):
            slot = pred_step['pokemon_idx']
            if pred_step['predictions']:
                top_preds = pred_step['predictions'][:5]
                stage_data = {'pokemon': [], 'probability': [], 'color': []}

                colors = plt.cm.viridis(np.linspace(0, 1, len(top_preds)))
                for pred_idx, (pokemon, prob) in enumerate(top_preds):
                    stage_data['pokemon'].append(pokemon[:12])
                    stage_data['probability'].append(prob)
                    stage_data['color'].append(colors[pred_idx])

                stages.append(stage_data)
                stage_names.append(f"Slot {slot}\n(Order {pred_step['propagation_order']})")

        if not stages:
            ax.text(0.5, 0.5, f"Game {row.get('game_id', 'Unknown')}: No valid predictions",
                    ha='center', va='center', transform=ax.transAxes)
            continue
        ...
        max_preds = max(len(stage['pokemon']) for stage in stages)

        for pred_rank in range(max_preds):
            x_offset = x_positions + pred_rank * bar_width - (max_preds - 1) * bar_width / 2
            heights = []
            colors = []
            labels = []

            for stage in stages:
                if pred_rank < len(stage['pokemon']):
                    heights.append(stage['probability'][pred_rank])
                    colors.append(stage['color'][pred_rank])
                    labels.append(stage['pokemon'][pred_rank])
                else:
                    heights.append(0)
                    colors.append('white')
                    labels.append('')

            bars = ax.bar(x_offset, heights, bar_width, color=colors, alpha=0.8,
        ...
        ax.set_title(f'Game {row.get("game_id", "Unknown")}: Prediction Evolution\n'
                     f'Initial Revealed: {initial_revealed}, Predicting from Slot {start_idx}')
        ax.set_xticks(x_positions)
        ax.set_xticklabels(stage_names)
        ax.set_ylim(0, max(max(stage['probability']) for stage in stages) * 1.2)
        ax.grid(True, alpha=0.3)

        actual_pokemon = []
        for seq_idx in range(1, 6):
            seq_col = f'p2_next_reveal_{seq_idx}'
            if seq_col in row and not pd.isna(row[seq_col]):
                actual_pokemon.append(row[seq_col])

        if actual_pokemon:
            ax.text(0.02, 0.98, f"Actual sequence: {', '.join(actual_pokemon[:3])}{'...' if len(actual_pokemon) > 3 else ''}",
    ...
    plt.close()
    print("Prediction evolution visualization saved to 'prediction_evolution.png'")

def create_prediction_confidence_distribution(df, models):
    print("\nCreating prediction confidence distribution visualization...")

    all_confidences = []
    all_orders = []
    all_slots = []

    sample_data = df.sample(n=min(200, len(df)), random_state=RANDOM_STATE)
    ...
    for _, row in sample_data.iterrows():
        ...
        for pred_step in prediction_chain:
            slot = pred_step['pokemon_idx']
            order = pred_step['propagation_order']

            for pokemon, prob in pred_step.get('predictions', []):
    ...
    ax1.hist(all_confidences, bins=50, alpha=0.7, color='skyblue', edgecolor='black')
    ...
    sns.boxplot(data=confidence_df, x='order', y='confidence', ax=ax2)
    ax2.set_xlabel('Propagation Order')
    ax2.set_ylabel('Prediction Confidence')
    ax2.set_title('Confidence by Propagation Order')
    ax2.grid(True, alpha=0.3)

    sns.boxplot(data=confidence_df, x='slot', y='confidence', ax=ax3)
    ax3.set_xlabel('Pokemon Slot')
    ax3.set_ylabel('Prediction Confidence')
    ax3.set_title('Confidence by Pokemon Slot')
    ax3.grid(True, alpha=0.3)

    pivot_data = confidence_df.groupby(['slot', 'order'])['confidence'].mean().unstack(fill_value=0)
    sns.heatmap(pivot_data, annot=True, fmt='.3f', cmap='YlOrRd', ax=ax4)
    ax4.set_xlabel('Propagation Order')
    ax4.set_ylabel('Pokemon Slot')
    ax4.set_title('Average Confidence by Slot and Order')

    plt.tight_layout()
    plt.savefig('prediction_confidence_distribution.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Prediction confidence distribution saved to 'prediction_confidence_distribution.png'")

def create_pokemon_frequency_analysis(df, models):
    print("\nCreating Pokemon frequency analysis...")

    predicted_pokemon = defaultdict(int)
    actual_pokemon = defaultdict(int)

    for _, row in df.iterrows():
        for seq_idx in range(1, 6):
            seq_col = f'p2_next_reveal_{seq_idx}'
            if seq_col in row and not pd.isna(row[seq_col]):
                actual_pokemon[row[seq_col]] += 1

    sample_data = df.sample(n=min(500, len(df)), random_state=RANDOM_STATE)
    ...
    for _, row in sample_data.iterrows():
        ...
        for pred_step in prediction_chain:
            if pred_step.get('predictions'):
                top_pokemon = pred_step['predictions'][0][0]
                predicted_pokemon[top_pokemon] += 1

    all_pokemon = set(list(predicted_pokemon.keys()) + list(actual_pokemon.keys()))
    comparison_data = []

    for pokemon in all_pokemon:
        comparison_data.append({
            'pokemon': pokemon,
            'predicted_freq': predicted_pokemon[pokemon],
            'actual_freq': actual_pokemon[pokemon]
        })

    comparison_df = pd.DataFrame(comparison_data)
    comparison_df = comparison_df.sort_values('actual_freq', ascending=False).head(20)
    ...
    ax1.bar(x - width/2, comparison_df['actual_freq'], width, label='Actual', alpha=0.8, color='lightblue')
    ax1.bar(x + width/2, comparison_df['predicted_freq'], width, label='Predicted', alpha=0.8, color='orange')
    ax1.set_xlabel('Pokemon')
    ax1.set_ylabel('Frequency')
    ax1.set_title('Top 20 Pokemon: Actual vs Predicted Frequencies')
    ax1.set_xticks(x)
    ax1.set_xticklabels(comparison_df['pokemon'], rotation=45, ha='right')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    comparison_df['ratio'] = np.where(comparison_df['actual_freq'] > 0,
                                      comparison_df['predicted_freq'] / comparison_df['actual_freq'],
                                      0)

    colors = ['red' if r < 0.5 else 'orange' if r < 1.5 else 'green' for r in comparison_df['ratio']]
    ax2.bar(x, comparison_df['ratio'], color=colors, alpha=0.7)
    ax2.axhline(y=1, color='black', linestyle='--', alpha=0.5)
    ax2.set_xlabel('Pokemon')
    ax2.set_ylabel('Predicted/Actual Ratio')
    ax2.set_title('Prediction vs Reality Ratio (1.0 = Perfect Match)')
    ax2.set_xticks(x)
    ax2.set_xticklabels(comparison_df['pokemon'], rotation=45, ha='right')
    ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('pokemon_frequency_analysis.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("Pokemon frequency analysis saved to 'pokemon_frequency_analysis.png'")
    ...
    if not all_predictions_df.empty:
        ...
        for slot in range(2, 7):
            ...
            for order, order_data in slot_data['by_propagation_order'].items():
                if order_data['total'] > 0:
                    order_accuracy_data.append({
                        'Slot': int(slot),
                        'Order': int(order),
                        'Accuracy': float(order_data['accuracy']),
                        'Count': int(order_data['total'])
                    })

        if order_accuracy_data:
            order_accuracy_df = pd.DataFrame(order_accuracy_data)

            plt.figure(figsize=(14, 8))
            pivot_table = order_accuracy_df.pivot(index='Slot', columns='Order', values='Accuracy')
            pivot_table = pivot_table.fillna(0).astype(float)
            ...
            sns.heatmap(count_pivot, annot=True, cmap='Greens', fmt='d', cbar_kws={'label': 'Number of Predictions'})
        ...
        all_predictions_df_clean = all_predictions_df.copy()
        all_predictions_df_clean['propagation_order'] = pd.to_numeric(all_predictions_df_clean['propagation_order'], errors='coerce')
        all_predictions_df_clean['is_correct'] = pd.to_numeric(all_predictions_df_clean['is_correct'], errors='coerce')
        all_predictions_df_clean = all_predictions_df_clean.dropna(subset=['propagation_order', 'is_correct'])

        if not all_predictions_df_clean.empty:
            order_accuracy = all_predictions_df_clean.groupby('propagation_order')['is_correct'].agg(['mean', 'count']).reset_index()

            if not order_accuracy.empty:
                plt.figure(figsize=(10, 6))
                order_accuracy['propagation_order_str'] = order_accuracy['propagation_order'].astype(str)
                plt.bar(order_accuracy['propagation_order_str'], order_accuracy['mean'], color='skyblue')
        ...
        if 'confidence' in all_predictions_df.columns:
            plt.figure(figsize=(10, 6))
            confidence_df = all_predictions_df.copy()
            confidence_df['confidence'] = pd.to_numeric(confidence_df['confidence'], errors='coerce')
            confidence_df['is_correct'] = pd.to_numeric(confidence_df['is_correct'], errors='coerce')
            confidence_df = confidence_df.dropna(subset=['confidence', 'is_correct'])
            ...
            if not confidence_df.empty:
                ...
                conf_accuracy = confidence_df.groupby('confidence_bin')['is_correct'].mean()
                conf_counts = confidence_df.groupby('confidence_bin').size()

                bin_labels = [f"{inter.left:.1f}-{inter.right:.1f}" for inter in conf_accuracy.index]
                plt.bar(range(len(conf_accuracy)), conf_accuracy.values, color='coral')
                ...
                for i, acc in enumerate(conf_accuracy.values):
                    if not np.isnan(acc):
        ...
        if 'initial_p2_revealed_count' in all_predictions_df.columns:
            revealed_df = all_predictions_df.copy()
            revealed_df['initial_p2_revealed_count'] = pd.to_numeric(revealed_df['initial_p2_revealed_count'], errors='coerce')
            revealed_df['propagation_order'] = pd.to_numeric(revealed_df['propagation_order'], errors='coerce')
            revealed_df['is_correct'] = pd.to_numeric(revealed_df['is_correct'], errors='coerce')
            revealed_df = revealed_df.dropna(subset=['initial_p2_revealed_count', 'propagation_order', 'is_correct'])

            if not revealed_df.empty:
                revealed_order_pivot = revealed_df.groupby(['initial_p2_revealed_count', 'propagation_order'])['is_correct'].agg(['mean', 'count']).reset_index()
                ...
                if not revealed_order_pivot.empty:
                    ...
                    revealed_accuracy_pivot = revealed_accuracy_pivot.fillna(0).astype(float)
                    ...
                    revealed_count_pivot = revealed_count_pivot.fillna(0).astype(int)

                    plt.figure(figsize=(12, 8))
                    sns.heatmap(revealed_count_pivot, annot=True, cmap='Greens', fmt='d', cbar_kws={'label': 'Number of Predictions'})
...
def create_sample_visualizations(df, models, num_samples_per_category=1):
    ...
    for revealed_count in range(0, 6):
        ...
        for i, (_, example_row_series) in enumerate(example_rows.sample(
                min(num_samples_per_category, len(example_rows)), random_state=RANDOM_STATE).iterrows()):
...
def main():
    ...
    if SAMPLE_SIZE is not None and SAMPLE_SIZE > 0 and SAMPLE_SIZE < len(df):
        ...
        df = sample_data_stratified(df, SAMPLE_SIZE, strat_col='p2_number_of_pokemon_revealed', random_state=RANDOM_STATE)
    else:
        print("\nUsing full validation dataset or SAMPLE_SIZE is not applicable.")
    ...
    print("\nCreating prediction evolution visualization...")
    try:
        create_prediction_evolution_visualization(df, models, sample_size=5)
    except Exception as e:
        print(f"Error during create_prediction_evolution_visualization: {e}")


    print("\nCreating prediction confidence distribution...")
    try:
        create_prediction_confidence_distribution(df, models)
    except Exception as e:
        print(f"Error during create_prediction_confidence_distribution: {e}")


    print("\nCreating Pokemon frequency analysis...")
    try:
        create_pokemon_frequency_analysis(df, models)
    except Exception as e:
        print(f"Error during create_pokemon_frequency_analysis: {e}")
